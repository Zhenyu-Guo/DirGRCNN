...
# from software.pytorch_DGCNN.lib.gnn_lib import _gnn_lib
from software.pytorch_DGCNN.lib.gnn_lib import GNNLIB
from software.pytorch_DGCNN.lib.pytorch_util import weights_init, gnn_spmm
...
class DGCNN(nn.Module):
    def __init__(self, output_dim, num_node_feats, num_edge_feats, latent_dim=[32, 32, 32, 1], k=30,
        ...
        self.k = k
        self.total_latent_dim = np.sum(self.latent_dim)
        conv1d_kws[0] = self.total_latent_dim
        print("self.num_edge_feats: ", self.num_edge_feats)
        print("self.num_node_feats:",  self.num_node_feats)

        self.conv_params = nn.ModuleList()
        self.conv_params.append(nn.Linear(num_edge_feats+num_node_feats, latent_dim[0]))
